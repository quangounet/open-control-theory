...
%TODO
% Add background from part I
% - Laplace transform and time response
% - stability properties and poles
% - system type
% - pole/zero compensation
% PI control by system type (do not re-derive the results)
% Lead compensator design by algebraic method
% Re-edit the tutorials
...
\begin{itemize}
  ...
  \item If at least one pole is in the open right half-plane, the
    system is \emph{unstable}.
\end{itemize}

\subsection{Transient behavior}

Once a system has been determined as stable, one might be interested
in its transient behaviork. The transient behavior depends on the
dominant poles.
...
Consider a feedback system as in the figure below.
...
Let $G$ be the open-loop transfer function. Assume that $G$ is written
as
\[
G=\frac{N(s)}{s^kD(s)},
\]
where $N$ and $D$ are not factorizable by $s$. Then the type of the
system is given by the integer $k$. The figure below shows the general
form of systems of types 1, 2 and 3.
...
Let us compute the steady-state error for a system of type $k$.
\[
...
feedforward loop. Note that pure derivative controllers are seldom
...
K(s+z_c)N(s)+D(s) = 0.
\]
We want the root locus of the new system to go through $s_\des$. The
angle criterion then yields 
\[
\angle D(s_\des)-\angle \left[(s_\des+z_c)N(s_\des)\right] =
\]
\[
\angle D(s_\des)-\angle N(s_\des) - \angle(s_\des+z_c) = \pi \mod 2\pi,
\]
i.e.
\[
\angle (s_\des+z_c) = \angle D(s_\des)-\angle N(s_\des) - \pi \mod 2\pi.
\]
...
Consider a system with forward transfer function $G(s)$. We first
design a PD controller $K_1(s+z_{c1})$ to meet the transients
specifications. Then, we design a PI controller
$K_2\frac{s+z_{c2}}{s}$ for the system with forward TF
...
resulting system will have a forward TF given by
\[
...
previously. The resulting system will have a forward TF given by
\[
...
\begin{example}
  ...
  and the forward loop gain of the system at hand are given by
  \[
