...
\begin{equation}
  \label{eq:sse}
  \mathrm{SSE} = \lim_{t\to\infty} e(t) = \lim_{s\to 0} sE(s).  
\end{equation}

The second equality sign comes from the application of the FVT and is
valid only if the system is \emph{stable}.
...
\begin{itemize}
\item For a system of type 0, let $K_p=\lim_{s\to 0}\frac{N_0(s)}{D_0(s)}$;
\item For a system of type 1, let $K_v=\lim_{s\to 0}\frac{N_1(s)}{D_1(s)}$;
\item For a system of type 2, let $K_a=\lim_{s\to 0}\frac{N_2(s)}{D_2(s)}$.
\end{itemize}

Then the steady-state errors are given by the table below.
...
\begin{table}[htp]
  ...
  \begin{tabular}{|c|c|c|c|}
    ...
    Type 3&0&0&0\\\hline
  \end{tabular}
  \caption{System type and steady-state error}
  \label{tab:systype}
\end{table}
