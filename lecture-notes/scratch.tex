...
\begin{tikzpicture}[auto, node distance=20mm,>=latex',every text node part/.style={align=center}]
  ...
  \node [block, right of=sum1, node distance=25mm] (pd) {$\vect{k}_p+\vect{k}_ds$};
  \node [sum, right of=pd, node distance=25mm] (sum3) {\plus};
  \node [input, above of=sum3, node distance=15mm] (xr) {};
  \node [block, right of=sum3, node distance=35mm] (robot) {Robot (with internal\\
    position controller)};
  ...
  % Lines
  \draw [->] (fr) -- node[pos=0.85] {\plus} node [pos=0]{$\vect{f}_\mathrm{ref}$} (sum1);
  \draw [->] (sum1) -- node {$\vect{f}_\mathrm{err}$} (pd);
  \draw [->] (pd) --  node {$\vect{x}_\mathrm{err}$} (sum3);
  \draw [->] (xr) --  node [pos=0, above]{$\vect{x}_\mathrm{ref}$} (sum3);
  \draw [->] (sum3) -- node {$\vect{x}_\mathrm{com}$} (robot);
  \draw [->] (robot) --  (robot -| env.west);
  \draw [<-] (ft_sensor) -- (ft_sensor -| env.west);
  \draw [->] (ft_sensor) -|  node[pos=0.95] {\minus} node[pos=0.75]  {$\vect{f}_\mathrm{sensed}$} (sum1);
\end{tikzpicture}
